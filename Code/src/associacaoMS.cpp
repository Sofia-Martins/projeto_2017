	...
	do {
		...
		for (unsigned int i = 0; i < associacoes.size(); i++) {
			if (siglaAssociacao == associacoes.at(i).first) {
				encontrou = true;
				std::cout
						<< "A sigla da associacao jÃ¡ esta a ser usada, tente outra! "
						<< std::endl << std::endl;
			}

			if (nomeAssociacao == associacoes.at(i).second) {
				encontrou = true;
				std::cout << "A associacao jÃ¡ existe! " << std::endl
						<< std::endl;
...
void AssociacaoMS::lerEmails()
{
	...
	in.get(carater);
	//extrair informaÃ§Ã£o do ficheiro
	while (!in.eof())
	{
		if (primeiraLeitura)
		{
...
void AssociacaoMS::lerGestores()
{
	...
	while (!in.eof())
	{
		...
		//buscar endereÃ§o de email do gestor
		getline(in, email);
		if(email == "")
			break;
		eliminateSpaces(email);
...
void AssociacaoMS::menuSessaoAssociado(unsigned int id)
{
	...
	//verifica que tipo de associado Ã© (contributor, subscriber, ou simplesmente um associado)
